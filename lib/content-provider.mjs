...
import { collectMarkdownFiles, getActiveContentDir, getWritableContentDir } from '../content-source.mjs'
...
function classifyEntry(dirPath, entry) {
  ...
  try {
    ...
    if (stat.isDirectory()) {
      return {
        kind: 'dir',
        size: dirStats(fullPath).size ?? null,
...
class FilesystemProvider {
  #readRoot() {
    return getActiveContentDir()
  }

  #writeRoot() {
    return getWritableContentDir()
  }

  #safePath(rootDir, relPath) {
  ...
  async readFile(relPath) {
    const filePath = this.#safePath(this.#readRoot(), relPath)
  ...
  async listMarkdownFiles() {
    return collectMarkdownFiles(this.#readRoot())
  }

  async listDirectory(relPrefix) {
    const dirPath = this.#safePath(this.#writeRoot(), relPrefix || '.')
    ...
    for (const entry of entries) {
      ...
      const resolved = classifyEntry(dirPath, entry)
      if (!resolved) continue
      if (resolved.kind === 'dir') {
        dirs.push({ name: entry.name, size: resolved.size, lastModified: resolved.lastModified })
      } else {
        files.push({ name: entry.name, size: resolved.size, lastModified: resolved.lastModified })
      }
    }
  ...
  async writeFile(relPath, content) {
    const filePath = this.#safePath(this.#writeRoot(), relPath)
  ...
  async createDirectory(relPath) {
    const dirPath = this.#safePath(this.#writeRoot(), relPath)
