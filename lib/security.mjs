...
// Defaults: show_toc=true, show_sitemap=true, show_sitemap_first=false, show_sitemap_siteroot=false
export function findDisplayConfig(filePath, rules, global) {
  const KEYS = ['show_toc', 'show_sitemap', 'show_sitemap_first', 'show_sitemap_siteroot']
  const best = {}
  ...
  return {
    showToc:             (best.show_toc             ? best.show_toc.show_toc                         : (global?.show_toc             ?? true))  !== false,
    showSitemap:         (best.show_sitemap         ? best.show_sitemap.show_sitemap                 : (global?.show_sitemap         ?? true))  !== false,
    showSitemapFirst:    (best.show_sitemap_first   ? best.show_sitemap_first.show_sitemap_first     : (global?.show_sitemap_first   ?? false)) === true,
    showSitemapSiteroot: (best.show_sitemap_siteroot? best.show_sitemap_siteroot.show_sitemap_siteroot:(global?.show_sitemap_siteroot ?? false)) === true,
  }
}
...
export function findIndexFile(filePath, rules, globalIndexFile) {
  ...
  for (const rule of rules) {
    ...
    const m = rule.match
    const matched = m.endsWith('/') ? (filePath.startsWith(m) || filePath === m.slice(0, -1)) : filePath === m
    if (matched && (!best || m.length > best.match.length)) best = rule
  }
